...
/**
 * <p>
 * Like WeakHashMap, except that the keys of the <em>n</em> most recently-accessed entries are kept as {@link SoftReference soft
 * references}. Accessing an element means creating it, or retrieving it with {@link #get(Object) get}. Because these
 * entries are kept with soft references, they will tend to remain even if their keys are not referenced elsewhere. But
 * if memory is short, they will be removed.</p>
 */
public class CacheMap extends WeakHashMap {

    /**
     * <p>
     * Create a <code>CacheMap</code> that can keep up to <code>nSoftReferences</code> as soft references.</p>
     *
     * @param nSoftReferences Maximum number of keys to keep as soft references. Access times for
     * {@link #get(Object) get} and {@link #put(Object, Object) put} have a component that scales linearly with
     * <code>nSoftReferences</code>, so this value should not be too great.
     *
