...
/**
 * <p>
 * This class represents a username/password based implementation of the {@link JMXAuthenticator} interface.</p>
 *
 * <p>
 * The set of username/password pairs is passed either as a filename which denotes a properties file on disk, or
 * directly as an instance of the {@link Properties} class. In both cases, the name of each property represents a
 * username, and the value of the property is the associated password. Thus, any given username either does not exist in
 * the properties or has exactly one password. The same password can be shared by several usernames.</p>
 *
 * <p>
 * If authentication is successful then an authenticated {@link Subject} filled in with a {@link JMXPrincipal} is
 * returned. Authorization checks will then be performed based on this <code>Subject</code>.</p>
 */
public class JMXPasswordAuthenticator implements JMXAuthenticator {

    /**
     * Creates an instance of <code>JMXPasswordAuthenticator</code> and initializes it from the specified properties
     * file.
     *
    ...
    public JMXPasswordAuthenticator(String pwFile) throws IOException {
        this.pwFile = pwFile;
        this.props = propertiesFromFile(pwFile);
    }

    /**
     * Creates an instance of <code>JMXPasswordAuthenticator</code> and initializes it from the specified properties
     * instance. This constructor makes a copy of the properties instance using its <code>clone</code> method and it is
     * the copy that is consulted to check the username and password of an incoming connection. The original properties
     * object can be modified without affecting the copy. If the {@link #refresh} method is then called, the
     * authenticator will make a new copy of the properties object at that time.
     *
    ...
    public JMXPasswordAuthenticator(Properties pwProps) {
        ...
        this.props = new Properties(pwProps);
    }

    /**
     * Authenticate the <code>MBeanServerConnection</code> client with the given client credentials.
     *
     * @param credentials the user-defined credentials to be passed in to the server in order to authenticate the user
     * before creating the <code>MBeanServerConnection</code>. This parameter must be a two-element
     * <code>String[]</code> containing the client's username and password in that order.
     *
     ...
     *
     * <p>
     * If this instance was created using the {@link
     * #JMXPasswordAuthenticator(String)} constructor to specify a file from which the entries are read, the file is
     * re-read.</p>
     *
     * <p>
     * If this instance was created using the {@link #JMXPasswordAuthenticator(Properties)} constructor then a new copy
     * of the <code>Properties</code> object is made.</p>
     *
     * @exception IOException if the file does not exist, is a directory rather than a regular file, or for some other
     * reason cannot be opened for reading.
     */
    public void refresh() throws IOException {
        synchronized (props) {
            if (pwFile == null) {
                props = new Properties(originalProps);
            } else {
