...
public class AdminClient implements ClientAdmin {
    ...
    /**
     * If a profile selector has been supplied in the map then use it. Otherwise, the default profile selection
     * algorithm implemented works as follows:
     * <p>
     * Upon reception of the HandshakeBeginMessage the client verifies that the profiles he wants to use, i.e. the ones
     * he specified in the environment map through the jmx.remote.profiles property are all present in the server's
     * supported profiles list. If false, the client sends a HandshakeErrorMessage to the server and closes the
     * connection. Otherwise, the client starts exchanging profile messages with the server for the selected profiles
     * following the order specified in the client's profile list.
     */
    ...
    /**
     * If a HandshakeErrorMessage is received then choose the appropriate exception to throw based on the error string
     * detail.
     */
    ...
    private final List profilesList = new ArrayList();
